---
title: "OpenAI - Tales of the Call Center Subreddit Report"
output:
  pdf_document:
    toc: no
  html_document:
...
# Load the CSV files
data <- read_csv("tfcc_top_20_summaries_of_summaries.csv")
comments_data <- read_csv("tfcc_comments_summaries_of_summaries.csv")
summary_data <- read_csv("tfcc_summary_details.csv")
...
print_topic_report <- function(topic_data, comment_data, summary_data, intro_summary) {
  ...
  #suppressWarnings(matrix(topic_data$theme, ncol = 3, byrow = TRUE))
  ...
  cat("### Brief Summary of Comments\n\n")
  cat(escape_markdown(comment_data$new_summary), "\n")
}
